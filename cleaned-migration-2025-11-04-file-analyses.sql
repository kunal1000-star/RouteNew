-- ============================================================================
-- CLEANED MIGRATION FILE: migration-2025-11-04-file-analyses.sql
...
CREATE TABLE IF NOT EXISTS IF NOT EXISTS file_analyses (
...
CREATE POLICY IF NOT EXISTS "Users can view their own file analyses" ON file_analyses
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own file analyses" ON file_analyses
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own file analyses" ON file_analyses
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete their own file analyses" ON file_analyses
    FOR DELETE USING (auth.uid() = user_id);

-- Users can only see their own file uploads
CREATE POLICY IF NOT EXISTS "Users can view their own file uploads" ON file_uploads
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own file uploads" ON file_uploads
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own file uploads" ON file_uploads
    FOR UPDATE USING (auth.uid() = user_id);

-- Users can only see their own analysis-study plan links
CREATE POLICY IF NOT EXISTS "Users can view their own study plan links" ON analysis_study_plan_links
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own study plan links" ON analysis_study_plan_links
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own study plan links" ON analysis_study_plan_links
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete their own study plan links" ON analysis_study_plan_links
    FOR DELETE USING (auth.uid() = user_id);
