-- ============================================================================
-- CLEANED MIGRATION FILE: migration-2025-11-04-ai-suggestions.sql
-- Generated: 2025-11-06T08:29:02.033Z
...
CREATE TABLE IF NOT EXISTS IF NOT EXISTS ai_suggestions (
...
-- RLS Policies
CREATE POLICY IF NOT EXISTS "Users can view own AI suggestions" ON ai_suggestions
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert own AI suggestions" ON ai_suggestions
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update own AI suggestions" ON ai_suggestions
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete own AI suggestions" ON ai_suggestions
  FOR DELETE USING (auth.uid() = user_id);

-- System can manage suggestions (for background processing)
CREATE POLICY IF NOT EXISTS "System can manage all AI suggestions" ON ai_suggestions
  FOR ALL USING (true);
